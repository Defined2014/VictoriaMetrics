...
func (fq *FakeQuerier) Query(_ context.Context, _ string, _ time.Time, _ *auth.Token) (Result, *http.Request, error) {
	fq.Lock()
	defer fq.Unlock()
	if fq.err != nil {
...
func (fqr *FakeQuerierWithRegistry) Query(_ context.Context, expr string, _ time.Time, _ *auth.Token) (Result, *http.Request, error) {
	fqr.Lock()
	defer fqr.Unlock()
